...
description: Review the WHOLE resulting module for cohesion, not just the diff, whenever a change grows an existing file, the moment a linter flags a file for size or complexity, or whenever you catch yourself copying a helper. Read the accreted file fresh and ask whether it still does one thing and does it once, or whether it has grown a second responsibility and now holds two helpers that compute the same thing under different names. Per-diff review optimizes the increment and never sees the whole it accretes into, which is how a clean file becomes a duplicated monolith one reasonable diff at a time. Fires even when a user only says a file feels big, is getting hard to hold in their head, or that they are about to add yet another function to a growing module. Not for sizing a unit of work before doing it (decompose-by-attention) or bundling changes into PRs (pr-sizing).
