...
description: Before building on a design, plan, or architecture that quietly assumes things about environment, dependencies, behavior, timing, or scale no one consciously chose, or when someone asks "what could break?" or "what did we assume?": inventory those unchosen beliefs, not explicit choices (decision-gate) or design-section coverage (design-completeness). Record each in a committed register with a falsifying condition ("breaks if X"), a fragility scored by damage if it breaks, and a detector. High-fragility entries route to derisk-gate or decision-gate; the rest carry a fallback. Not name-vs-behavior lies in code (footgun-register).
