...
description: A spec, PRD, design doc, constitution, or numbered requirements list and a test suite both exist, and you need to know which requirements carry a test: "which requirements have no test", "coverage gaps versus the spec", "untested requirements". Every requirement, rule, or acceptance criterion MUST trace to at least one test that would fail if it were violated, through a mechanically checkable link: a shared identifier the test names, not a human's claim of coverage. Reports requirements with no test (verification gaps) and tests tied to no requirement (scope drift), before trusting a green suite as evidence the spec is met. Not for brownfield work with no stated spec (there the recovered characterization tests are the spec), nor for whether the design is complete enough to trace to (that is design-completeness); this starts once requirements are stated.
